...
public class PayBillDAOImpl implements PayBillDAO {
	...
	public List<Bill> showBillByStatus(String username, String billType) {
		...
		Root<Bill> from = criteriaQuery.from(Bill.class);
		criteriaQuery.where(criteriaBuilder.equal(from.get("billStatus"), "Pending"),
				criteriaBuilder.equal(from.get("billType"), billType),
