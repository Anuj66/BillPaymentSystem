...
<script>
	function validateForm() {
		...
		var username = documment.forms["ElectricityBill"]["username"];
		var pan = documment.forms["ElectricityBill"]["pan"];
		var amount = documment.forms["ElectricityBill"]["amount"];
		var cardName = documment.forms["ElectricityBill"]["cardName"];
		var cardCVV = documment.forms["ElectricityBill"]["cardCVV"];
		var cardNumber = documment.forms["ElectricityBill"]["cardNumber"];
		var letters = /^[A-Za-z]+$/;
		...
		if (pan.value == "") {
			window.alert("Please enter your pan:");
			pan.focus();
			return false;
		}
		if (amount.value == "") {
		...
		if (cardName.value == "") {
			window.alert("Please enter your  card name:");
			cardName.focus();
			return false;
		}
		if (cardCVV.value == "") {
	...
	function validateCardNumber() {
		...
		if (isNaN(x) || x.length != 12) {
			...
			document.getElementById("validCardNumber").innerHTML = text;
		} else {
			document.getElementById("validCardNumber").innerHTML = "";
		}
	}

	function validateCardCVV() {
		var x, text;
		x = document.getElementById("cardCVV").value;
		if (isNaN(x) || x.length != 3) {
			text = "Should be 3 digit number";
			document.getElementById("validCardCVV").innerHTML = text;
		} else {
			document.getElementById("validCardCVV").innerHTML = "";
		}
	}

	function validateCardName() {
		var x, text;
		x = document.getElementById("cardName").value;
		var letters = /^[A-Za-z]+$/;
		if (x.match(letters) || x.match(" ")) {
			document.getElementById("validCardName").innerHTML = "";
		} else {
			text = "Should contain only alphabets";
			document.getElementById("validCardName").innerHTML = text;
		}
	}
	...
	function validateamount() {
		...
		if (isNaN(x)) {
			text = "Should only contain numeric digits"
			document.getElementById("validamount").innerHTML = text;
		} else {
			document.getElementById("validCardName").innerHTML = "";
		}
	}
...
<body class="w3-light-grey"
	background="linear-gradient(-25deg, #616161 0%, #96B7C4 100%)">
	...
	<div class="w3-content w3-margin-top" style="max-width: 1400px;">
		...
		<div class="w3-row-padding">
			...
			<div class="w3-third">
				...
				<div class="w3-white w3-text-grey w3-card-4">
					...
					<div class="w3-container">
						...
						<p>
							<i class="fa fa-home fa-fw w3-margin-right w3-large w3-text-teal"></i>${user.aadhar}</p>
						<p>
							<i
								class="fa fa-envelope fa-fw w3-margin-right w3-large w3-text-teal"></i>${user.emailId}</p>
						<p>
							<i
								class="fa fa-phone fa-fw w3-margin-right w3-large w3-text-teal"></i>${user.contactNo}</p>

						<br>
						<center>
							<a href="back"><button type="button" class="btn btn-danger">Back</button></a>
						</center>
						<br>
					</div>
				</div>
			...
			<div class="w3-twothird">
				...
				<div class="w3-third">
					<form name="ElecrticityBill" action="saveBill" method="POST"
						modelAttribute="bill">

						<input id=billId type="hidden" value="${bill.billId}"
							name="billId" required> <label for="username"
							style="border: none; font-size: 1em; color: #818181; padding: 12px; margin-left: 1em">User
							Name</label><br> <input type="text" id="username" name="username"
							placeholder="user id" value="${user.username}"
							style="border: none; font-size: 1em; color: #818181; padding: 12px;"
							onkeyup="validateUsername()" readonly> <label
							id="validUsername"
							style="border: none; font-size: 1em; color: #818181; padding: 12px; margin-left: 1em"></label>
						<br>
						<br> <label for="pan"
							style="border: none; font-size: 1em; color: #818181; padding: 12px; margin-left: 1em">Pan
							Card No</label> <input type="text" id="pan" name="pan"
							placeholder="pan card no" value="${user.pan}"
							style="border: none; font-size: 1em; color: #818181; padding: 12px;"
							onkeyup="validatePan()"> <label id="validPan"
							style="border: none; font-size: 1em; color: #818181; padding: 12px; margin-left: 1em"></label>
						<br> <input type="hidden" id="billType" name="billType"
							value="Electricity"
							style="border: none; font-size: 1em; color: #818181; padding: 12px;"><br>

						<label for="amount"
							style="border: none; font-size: 1em; color: #818181; padding: 12px; margin-left: 1em"
							required>Amount to be paid</label> <br> <input type="text"
							id="amount" name="amount" placeholder="250.00"
							value="${bill.amount}"
							style="border: none; font-size: 1em; color: #818181; padding: 12px; margin-left: 0em"
							" onkeyup="validateamount()" required> <label
							id="validamount"
							style="border: none; font-size: 1em; color: #818181; padding: 12px; margin-left: 1em"></label>
						<br> <br>
						<label id="Select Payment Through" style="border: none; font-size: 1em; color: #818181; padding: 12px; margin-left: 1em">Select Payment Through</label>
						
						<div class="custom-select" style="width: 200px;">
							<select name="paymentThrough" selected="${bill.paymentThrough}"
								required>
						...
						<br>
						<label style="border: none; font-size: 1em; color: #818181; padding: 12px;">Comments(Bill Info)
 						</label>  <textarea rows="4" cols="50" name="comment"maxlength="100" placeholder="Enter comment here(max words 100)"
						style="border: none; font-size: 1em; color: #818181; width:250px; height: 100px; 
						padding:10px;">${bill.comment}</textarea> 
				</div>
				<div class="w3-twothird">


					<label
						style="border: none; font-size: 1em; color: #818181; padding: 12px; margin-left: 10em">Card
						Holder Name</label> <br> <input type="text" id="cardName"
						name="cardName" value="${bill.cardName}"
						style="border: none; font-size: 1em; color: #818181; padding: 12px; margin-left: 10em"
						onkeyup="validateCardName()" required> <label
						id="validCardName"
						style="border: none; font-size: 1em; color: #818181; padding: 12px; margin-left: 10em"></label>
					<br> <br> <label
						style="border: none; font-size: 1em; color: #818181; padding: 12px; margin-left: 10em">Credit
						Card CVV</label> <br> <input type="password" id="cardCVV" name="cardCVV"
						value="${bill.cardCVV}"
						style="border: none; font-size: 1em; color: #818181; padding: 12px; margin-left: 10em"
						onkeyup="validateCardCVV()" required> <label
						id="validCardCVV"
						style="border: none; font-size: 1em; color: #818181; padding: 12px; margin-left: 10em"></label>
					<br> <br> <label
						style="border: none; font-size: 1em; color: #818181; padding: 12px; margin-left: 10em">Credit
						Card number</label> <br> <input type="text" id="cardNumber"
						maxlength="12" name="cardNumber" placeholder="1111-2222-3333-4444"
						value="${bill.cardNumber}"
						style="border: none; font-size: 1em; color: #818181; padding: 12px; margin-left: 10em"
						onkeyup="validateCardNumber()" required> <label
						id="validCardNumber"
						style="border: none; font-size: 1em; color: #818181; padding: 12px; margin-left: 10em"></label>
					<br> <br> 
					<label id="Select vendor" style="border: none; font-size: 1em; color: #818181; padding: 12px; margin-left: 10em">Select Vendor</label>
					...
					<input type="submit" value="Save & Exit" name="save"
						formaction="saveForm" onsubmit="return validateForm()"
						formmethod="Post"
					...
					<input type="submit" value="Pay Bill" name="pay"
						formaction="saveBill" formmethod="post"
